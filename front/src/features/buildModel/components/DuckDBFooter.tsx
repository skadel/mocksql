 ...
 */
const DuckDBFooter: React.FC<{ dialectLabel?: string }> = ({ dialectLabel = 'BigQuery' }) => {
  ...
  >
    ...
    <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: '5px', color: '#8a5a00' }}>
      ...
      <Typography sx={{ fontSize: 11.5, fontWeight: 700, color: '#8a5a00' }}>{t('footer.local')}</Typography>
    </Box>
    <Typography sx={{ fontSize: 11.5, color: '#6b8287' }}>
      · {t('footer.transpiled', { dialect: dialectLabel })}
    </Typography>
    <Box sx={{ ml: 'auto', display: 'inline-flex', alignItems: 'center', gap: 1 }}>
      <Typography sx={{ fontSize: 11.5, fontWeight: 600, color: '#23a26d' }}>{t('footer.zero_billed')}</Typography>
    </Box>
  </Box>
  );
};
