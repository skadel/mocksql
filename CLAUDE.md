...
back/
  ...
  build_query/         # cœur du système : graph LangGraph
    ...
    conversational_agent.py # correction via outils (patch/add/remove row, run_cte) — chat utilisateur ET boucle retry auto bad_data
...
**Boucle evaluate → conversational_agent (correction incrémentale)** : si `evaluation_feedback == "bad_data"` (ce qui inclut `empty_results`) et `gen_retries > 0` → `bad_data_to_agent` pose `auto_correct=True` → `conversational_agent`. L'agent prend sa **branche de correction auto** (prioritaire sur un `input` périmé grâce à `auto_correct`), reçoit le diagnostic niveau-étape (CTE trace dans le message RESULTS), et corrige de façon **ciblée** via ses outils (`patch_test_field` / `add_test_row` / `remove_test_row`), réservant `update_test_data` (régénération complète) aux cas où le patch ciblé est impossible. Il **décrémente lui-même** `gen_retries`. Retries épuisés → `bad_data_exhausted` → `history_saver`.

> **Garde-fou anti-retry-mort** : si l'agent n'émet aucun outil actionnable en boucle `bad_data`, `route_agent_output` retombe sur le `generator` (régénération complète) au lieu de `history_saver` — le rebranch n'est ainsi jamais pire que l'ancien comportement de regen.
>
> **Historique** : cette boucle passait par le `generator` (regen complet, sans état → tendance au thrashing : sorties quasi identiques d'un retry à l'autre). Rebranchée sur l'agent pour une correction incrémentale (garde l'acquis, `run_cte` inspecte, ne touche que le bloqueur).
...
- **`evaluation_feedback`** : quand le verdict est `Insuffisant`, `test_evaluator` classifie la cause en `"bad_data"` (données incorrectes → correction incrémentale via le `conversational_agent`, entrée `bad_data_to_agent`) ou `"bad_assertions"` (assertions mal définies → pas de relance automatique). Ce champ est dans `QueryState` et est utilisé par `route_evaluator`.
...
- **Profil = cache PII séparé et gitignoré** : le profil + `sample_values` contiennent des **valeurs brutes de l'entrepôt** (`top_values`, `left_key_sample`…) = PII. Ils sont stockés dans **`.mocksql/profile.json` (gitignoré, jamais commité)**, distinct du `schema_cache.json` (schéma seul, commité, sans valeurs). Le réplay (`mocksql test` / `cli/test_runner.py`) ne lit **pas** le profil, mais le `schema_cache.json` est **obligatoire** : le replay rejoue avec le **vrai schéma de l'entrepôt** (fidélité prod) et **n'infère jamais** les types depuis les lignes synthétiques — inférer masquerait un bug de type réel (ex. colonne date-like → VARCHAR → « Cannot compare VARCHAR and DATE »). Toute table présente dans les données d'un test mais absente du cache lève `SchemaMissingError` (`test_runner.py`), avec un message pointant vers `mocksql refresh-schemas -t …`. `_resolve_model_schemas` ne fait donc plus de fallback d'inférence. Voir `models/schemas.py` (`get/save_profile`, `_migrate_legacy_profile` qui sort le profil inline d'un `schema_cache.json` legacy au chargement) et `storage/config.py:get_profile_cache_path` (surcharge via `profile_cache` dans `mocksql.yml`, env `PROFILE_CACHE_PATH` prioritaire). Sur projet repris (clone, `profile.json` absent) : `check_profile` voit toutes les colonnes manquantes → la boîte de dialogue auto-profile existante repropose le profilage (sauf `profile_skipped` commité dans `tests/`).
...
- **Budget de profiling (auto-profiling sans friction)** : `storage/config.py:get_profile_budget_tb()` lit `profile_budget_tb` (`mocksql.yml` / env `PROFILE_BUDGET_TB`) — budget de **scan BigQuery en To**. `build_profile_request(..., budget_tb=...)` (`profile_checker.py`) estime chaque requête de profiling (dry-run par requête via `build_profile_queries_labeled`, qui étiquette chaque requête par table/`jointures`/`expression dérivée`) et **diffère** celles au-dessus du budget : `profile_queries` ne contient que les requêtes sous budget, `deferred` liste les scopes différés. Le front (`QueryChatComponent`) : si `profile_budget_tb` est configuré (renvoyé par `check-profile`) il l'applique d'office (one-click « Générer ») ; sinon il demande la valeur dans le popup (champ « Budget de scan », mémorisé en `localStorage.profileBudgetTb`, défaut 0.3). Tables différées → bannière « Profil partiel » + bouton « Compléter le profil » (`handleCompleteProfile` reprofile **sans** budget). `budget_tb=None`/absent = comportement historique (profile tout). Budget BigQuery only (DuckDB/Postgres = gratuit). La notification de fin de génération existe déjà (`Notification` API, effet sur transition `loading`).
...
- **Langue de sortie (`language`)** : réglage unique de `mocksql.yml` (`language: en` / `fr`, **défaut anglais**, env `MOCKSQL_LANGUAGE` en repli, valeur inconnue → anglais). Pilote À LA FOIS (1) la langue des textes **générés par le LLM** — noms de tests, descriptions, verdicts, réponses de chat, suggestions — via `storage/config.py:output_language_directive()` **préfixé au system prompt** de chaque nœud user-facing (`generate_data_prompt`/`update_data_prompt`, evaluator + assertion corrector/regen dans `examples_executor.py`, `assertion_corrector.py`, `conversational_agent.py`, `final_response_node.py`, `suggestions_node.py`) ; et (2) le **défaut de l'UI** front, exposé via `GET /api/config` et appliqué au démarrage par `applyBackendLanguage` (`front/src/i18n.ts`) **tant que l'utilisateur n'a pas choisi** explicitement via le sélecteur (persisté en `localStorage.mocksql_language`, qui prime). Les prompts restent rédigés en français : la directive précise que les exemples few-shot ne fixent que le FORMAT, pas la langue. Les **tags** (`Logique métier`, `Null checks`…) sont des identifiants, jamais traduits.
