...
`'2024-03-01T00:00:00'`. `_build_result_schema_block` doit rendre le type DuckDB RÉEL (via
...
`_build_result_sample_block` doit plafonner par LIGNES ENTIÈRES (jamais de troncature
...
from build_query.examples_executor import (
    _build_result_sample_block,
    _build_result_schema_block,
)
...
def test_schema_block_uses_real_duckdb_types_via_describe():
    ...
    block = _build_result_schema_block(df, con=con, view_name="__result__test")
...
def test_schema_block_falls_back_to_pandas_dtypes_without_con():
    df = _lying_result_df()
    block = _build_result_schema_block(df)
...
def test_schema_block_empty_df():
    block = _build_result_schema_block(pd.DataFrame())
...
def _wide_result_df(n_rows: int = 200) -> pd.DataFrame:
...
def test_sample_block_small_result_is_untruncated():
    df = _wide_result_df(10)
    block = _build_result_sample_block(df)
...
def test_sample_block_large_result_is_capped_by_whole_rows():
    df = _wide_result_df(200)
    block = _build_result_sample_block(df, max_rows=60)
...
def test_sample_block_prioritizes_subject_rows():
    df = _wide_result_df(200)
    block = _build_result_sample_block(
        df, test_description="focus sur l'indicateur nb_cartes", max_rows=6
    )
