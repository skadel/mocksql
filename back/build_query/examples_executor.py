...
class _Assertion(BaseModel):
    description: str = Field(
        description=(
            f"Phrase en {output_language_name()}, courte (max 12 mots), décrivant l'assertion en termes "
            "métier, lisible par un responsable non-développeur, même "
            "si les colonnes sont dans une autre langue. Sans noms de colonnes/CTEs ni mots-clés SQL. "
            + (
                "✓ Bon : 'Le montant total est toujours positif.' "
                "'Chaque commande appartient à un client actif.' "
                if get_language() == "fr"
                else "✓ Good: 'The total amount is always positive.' "
                "'Every order belongs to an active customer.' "
            )
...
def _cardinality_pin(row_count: int) -> Dict[str, Any]:
    ...
    description = (
        f"The result contains exactly {row_count} row(s)."
        if get_language() == "en"
        else f"Le résultat contient exactement {row_count} ligne(s)."
    )
    return {
...
async def _generate_assertions_and_evaluate(
    ...
    system_content = (
        output_language_directive()
        + "\n\n"
        + """Tu es un expert en tests SQL dbt-style avec DuckDB. À partir d'un \
...
En bref :
- `description` : phrase métier en """
        + output_language_name()
        + """, ≤12 mots, sans nom de colonne/CTE ni mot-clé SQL.
...
- `explanation` : une phrase ≤20 mots, FACTUELLE (jamais de superlatif « parfaitement », « idéal »,
  « parfait »), dans la langue de sortie indiquée en tête de prompt, lisible par un responsable métier — sans noms
...
"""
            if get_language() == "fr"
            else """  ✓ "The data correctly covers the nominal scenario."
  ✓ "The input values do not produce the expected result for this edge case."
"""
        )
...
Réponds uniquement avec l'objet structuré demandé.
    ...
    try:
        ...
        # Repli gracieux si l'appel LLM échoue : la requête DuckDB est valide mais on ne
        # peut ni générer d'assertions ni juger la qualité. Le schéma impose `reasoning` +
        # ≥1 assertion → un objet vide lèverait ValidationError, une exception NON attrapée
        # qui casserait le graphe (le crash même que ce repli est censé absorber). On émet
        # donc un pin de cardinalité BRUT `COUNT(*) = N` : `_is_bare_rowcount_pin` le
        # déduplique en aval (assertion_generator), ne laissant que le pin déterministe →
        # dégradation propre « cardinalité pincée, qualité non évaluée ».
        fr = get_language() == "fr"
        return _AssertionsAndEvaluation(
            reasoning=(
                "Évaluation LLM indisponible : repli sur le pin de cardinalité."
                if fr
                else "LLM evaluation unavailable: falling back to the cardinality pin."
            ),
            assertions=[
                _Assertion(
                    description=(
                        "Le résultat conserve sa cardinalité."
                        if fr
                        else "The result keeps its cardinality."
                    ),
                    expected_condition=f"COUNT(*) = {row_count}",
                    quantifier="aggregate",
                )
            ],
            verdict="Bon",
            explanation="Évaluation indisponible." if fr else "Evaluation unavailable.",
        )
...
async def _regenerate_assertion(
    ...
    system_content = (
        output_language_directive()
        + "\n\n"
        + """Tu es un expert en tests SQL DuckDB dbt-style. Une assertion a \
...
- Recopie la `description` d'origine À L'IDENTIQUE (sans la traduire ni la reformuler) : seul le SQL était cassé.
...
async def _fix_logically_failing_assertions(
    ...
    system_content = (
        output_language_directive()
        + "\n\n"
        + """Tu es un expert en tests SQL DuckDB dbt-style. Tu viens de générer \
...
**Règle de la `description` (si tu régénères une assertion) :** phrase courte
(max 12 mots), en langage métier, dans la langue de sortie indiquée en tête de prompt, sans noms de colonnes/CTEs ni mots-clés SQL.
