...
**Dérivabilité SQL — vérifie AVANT d'affirmer (règle non négociable).** Chaque symptôme énoncé doit être réellement PRODUIT par le SQL fourni. Avant de l'écrire, repère la clause qui le générerait ; si une garde (`CASE WHEN … ELSE …`, sentinelle `COALESCE(…, valeur)`, `NULLIF`, filtre `WHERE`) le NEUTRALISE, le symptôme naïf est FAUX. Énonce alors le symptôme RÉEL de cette neutralisation (« le cas passe inaperçu », « est remplacé par une valeur sentinelle », « n'apparaît jamais dans le rapport ») ou abandonne le cas. N'affirme JAMAIS un symptôme que le SQL empêche de se produire (ex. « une série stable est signalée comme anormale » alors qu'un `CASE`/`COALESCE` la met justement à l'écart).
