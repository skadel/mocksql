...
async def retrieve_existing_tests(session_id: str, state) -> list:
    ...
    2. DB test_results column in models table
    """
    ...
    # 2. Persistent DB storage
    from models.database import query as db_query
    from common_vars import MODELS_TABLE_NAME

    rows = await db_query(
        f"SELECT test_results FROM {MODELS_TABLE_NAME} WHERE session_id = $1",
        (session_id,),
    )
    if rows and rows[0].get("test_results"):
        try:
            result = json.loads(rows[0]["test_results"])
