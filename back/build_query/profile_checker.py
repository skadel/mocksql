...
def _load_model_profile(session_id: str) -> Optional[dict]:
    """Retrieve the stored profile from the test file."""
    test = get_test(session_id)
    if not test:
        return None
    return test.get("profile") or None


def _save_model_profile(session_id: str, profile: dict) -> None:
    """Persist the profile in the test file."""
    update_test(session_id, {"profile": profile})
...
async def check_profile(state: QueryState) -> dict:
    ...
    # 1. Load stored profile from test file
    profile = _normalize_profile(
        _load_model_profile(session_id) if session_id else None
    ) or {
        "tables": {},
        "joins": [],
    }
